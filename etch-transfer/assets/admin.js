(function ($) {
    ...
    $('#et-export-multi-btn').on('click', function () {
        var ids = $('.et-multi-check:checked').map(function () { return this.value; }).get();
        ...
        $.post(etchTransfer.ajaxUrl, { action: 'etch_export_multi', nonce: etchTransfer.nonce, ids: ids })
        .done(function (res) {
            if (res.success) {
                var p = res.data;
        ...
        reader.onload = function (e) {
            try {
                ...
                if (isMulti && data.items) {
                    // Summarise by post type
                    var typeCounts = {};
                    data.items.forEach(function (item) {
                        var t = item.source_post_type || 'unknown';
                        // Use friendly label where possible
